...
def get_energy_reference(
    ...
    if isinstance(reference, (str,)):
        ...
        shift = float(reference)
        reference = "user-specified"
    else:
        reference = None

    # chooses reference automatically
    if reference == None:
        metallic = bandgap < 0.05  # eV
        if metallic:
    ...
    if reference == "middle":
        ...
        shift = 0.0
    elif reference == "user-specified":
        logger.info("Reference energy set to {:.4f} eV.".format(shift))
    else:
        shift = 0.0

    if output_level == "MD_light" and reference != "fermi level":
        logger.warning(
